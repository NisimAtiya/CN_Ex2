...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start (1)
        # * Fill in end (1)
        proxy_socket.bind((proxy_host,proxy_port)) #open a socket to start a transmittion between the prxoy and client
        proxy_socket.listen()# open the server to broadcast a tcp requests
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                # * Fill in end (2)
                client_socket, address = proxy_socket.accept()#command to proxy give client prommition through and start tcp commonication

                # Create a new thread to handle the client request
                thread = threading.Thread(target=client_handler, args=(client_socket, address, server_adress))
                thread.start()
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            data = client_socket.recv(8192) #How big can be the information received
            if not data:
                break
            try:
                # Process the request
                ...
                client_socket.sendall(response)#send entire buffer to client
                break
            except Exception as e:
