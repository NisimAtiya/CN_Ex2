...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        server_socket.bind((host,port)) #open a socket to start a transmittion between the server and client
        server_socket.listen() # open the server to broadcast a tcp requests
        ...
        while True:
            try:
                ...
                client_socket,address =server_socket.accept() ##command to server give client prommition through and start tcp commonication
